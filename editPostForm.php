            ...
            <div class="col-lg-8">
                ...
                <form action="editPost.php" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <select name="category_id" class="form-control" id="categoryInput">
                            <?php foreach ($categories as $cat) : ?>
                                <option <?php if ($cat['id'] == $post['category_id']) echo 'selected' ?> value="<?= $cat['id'] ?>"> <?= $cat['name'] ?> </option>
                            <?php endforeach; ?>
                        </select>
                    </div>

                    <?php $selected_tags = getPostTags($post['id']); ?>
                    <select name="tag_id[]" class="form-control" id="tagInput" multiple>
                        <?php foreach ($tags as $tag) : ?>
                            <option value="<?= $tag['id'] ?>" <?= in_array($tag['id'], $selected_tags) ? 'selected' : '' ?>><?= $tag['tag_name'] ?></option>
                        <?php endforeach; ?>
                    </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="hidden" name="old_image_name" value="<?= $post['image'] ?>">
                    </div>
